-- COMPLETE SERVICES MIGRATION SCRIPT
-- This script consolidates all services data into the services table and cleans the profiles table

-- Step 1: Add content management fields to existing services table
...
-- Step 2: Insert content management record with data from profiles table (if exists)
...
-- Step 3: If no profiles exist, insert default content management record
INSERT INTO `services` (
    `title`,
    `description`,
    `icon`,
...
-- Step 4: Clean up profiles table by removing all services-related fields
ALTER TABLE `profiles` 
DROP COLUMN IF EXISTS `services_section_badge`,
DROP COLUMN IF EXISTS `services_section_title`,
DROP COLUMN IF EXISTS `services_section_description`,
DROP COLUMN IF EXISTS `services_button_text`,
DROP COLUMN IF EXISTS `services_page_title`,
DROP COLUMN IF EXISTS `services_page_description`,
DROP COLUMN IF EXISTS `services_benefit_1_text`,
DROP COLUMN IF EXISTS `services_benefit_1_icon`,
DROP COLUMN IF EXISTS `services_benefit_2_text`,
DROP COLUMN IF EXISTS `services_benefit_2_icon`,
DROP COLUMN IF EXISTS `services_benefit_3_text`,
DROP COLUMN IF EXISTS `services_benefit_3_icon`,
DROP COLUMN IF EXISTS `work_process_title`,
DROP COLUMN IF EXISTS `work_process_description`,
DROP COLUMN IF EXISTS `work_process_steps`,
DROP COLUMN IF EXISTS `services_cta_title`,
DROP COLUMN IF EXISTS `services_cta_description`,
DROP COLUMN IF EXISTS `services_cta_primary_text`,
DROP COLUMN IF EXISTS `services_cta_secondary_text`;

-- Step 5: Drop the services_management table if it exists (since we're consolidating into services table)
DROP TABLE IF EXISTS `services_management`;

-- Verification queries (uncomment to check results)
-- SELECT COUNT(*) as total_services FROM services;
-- SELECT COUNT(*) as content_management_records FROM services WHERE is_content_management = TRUE;
-- SELECT COUNT(*) as service_cards FROM services WHERE is_content_management = FALSE;
-- DESCRIBE profiles;
-- DESCRIBE services;
